    ...
    public class BCFTopic : BindableBase
    {
        ...
        private void Viewpoints_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Remove)
            {
                foreach (var removedViewpoint in e.OldItems)
                {
                    ...
                    // Remove from markup
                    Markup.Topic.Viewpoints.Remove(Markup.Topic.Viewpoints.First(v => v.Guid == ((VisualizationInfo) removedViewpoint).Guid));
                }
            }
            if (e.Action == NotifyCollectionChangedAction.Add)
            {
                ...
                foreach (var addedViewpoint in e.NewItems)
                {
                    ...
                    if (Markup.Topic.Viewpoints.Any(v => v.Guid == ((VisualizationInfo) addedViewpoint).Guid))
                    {
                        // Already known, just make sure the viewpoint reference is set correctly.
                        Markup.Topic.Viewpoints.First(v => v.Guid == ((VisualizationInfo) addedViewpoint).Guid).Viewpoint = "Viewpoint_" + ((VisualizationInfo) addedViewpoint).Guid + ".bcfv";
                    }
                    else
                    {
                        // Add to markup, viewpoint is not previously known
                        Markup.Topic.Viewpoints.Add(new ViewPoint
                        {
        ...
        public void AddOrUpdateSnapshot(string viewpointGuid, byte[] snapshotData)
        {
            ...
            else
            {
                ...
                // Add in Markup
                Markup.Topic.Viewpoints.First(v => v.Guid == viewpointGuid).Snapshot = "Snapshot_" + viewpointGuid + ".png";
            }
        }
